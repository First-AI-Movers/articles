...
def test_ingest_airtable_add_paths_include_llms_index():
    """The ingest workflow's peter-evans/create-pull-request `add-paths:`
    must include `llms-index.txt` so the corpus-index file generated by
    `tools/rebuild_local.py::build_llms_index` actually lands on the PR.

    Without this entry the file would regenerate inside the runner but
    never reach the PR, leaving `tools/check_generated_artifacts.py`
    permanently red on every fresh ingest run.
    """
    wf = _load_yaml("ingest-airtable.yml")
    steps = wf["jobs"]["ingest"]["steps"]
    pr_steps = [
    ...
    raw = (pr_steps[0].get("with") or {}).get("add-paths") or ""
    paths = [ln.strip() for ln in str(raw).splitlines() if ln.strip()]
    assert "llms-index.txt" in paths, (
        "ingest-airtable.yml `add-paths:` must include `llms-index.txt`. "
        f"Got: {paths!r}"
    )
