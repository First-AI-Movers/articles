...
| **E39** | Multilingual variants for top-20 articles (ES/FR/DE/NL/PT) | **E39b pilot done in PR #123.** 1 article × 5 languages (es, fr, de, nl, pt) with build integration (hreflang, self-canonical, `inLanguage` JSON-LD), review-gated workflow (`tools/translate_articles.py` + `translations.json` sidecar), and 39 rendering/translation tests. **E39c top-20 rollout remains pending** — awaits GoatCounter traffic data or citation-graph fallback, and monthly DeepL Free 500K-char budget planning. Full pipeline: `tools/translate_articles.py` calls DeepL API → produces `articles/<slug>/article.{es,fr,de,nl,pt}.md` → human review on a per-language basis. Adds `hreflang` markup, `inLanguage` JSON-LD, per-language sitemap entries. **3-10× citation reach for the highest-leverage articles, no new writing required.** | 📱 | M |
...
38. **E39** — multilingual variants. **E39b pilot (1 article × 5 languages) done in PR #123.** E39c top-20 rollout remains pending (awaits traffic data or citation-graph fallback).
