...
| ~~E35a~~ | ~~Multi-length structured summaries — infrastructure~~ | ✅ **Done as infrastructure.** Optional `summary_short`, `summary_medium`, `summary_long`, `summary_reviewed_at`, `summary_model` fields in `metadata.json` and `article_schema.json`. `tools/build_summaries.py` with mock/anthropic/openai/manual providers, `--dry-run` (default), `--write-review-files`, `--apply-approved`, `--allow-partial`, and `--allow-network` safety gate. Human review workflow via `summaries/<slug>.review.md` (draft → approved). Build integration: `index.json` passthrough, JSON-LD `description`, `llms-full.txt` headers. 30 tests. `docs/SUMMARIES.md` shipped. **No real summaries generated; no article metadata modified.** Pilot (E35b) pending human review of first batch. | 📱 | M |
