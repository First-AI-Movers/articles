...
3. **Supply chain — governed `@vN` policy (not a gap).** This repo **deliberately** pins actions to **major-version tags** (`actions/checkout@v6`, `actions/deploy-pages@v5`, `peter-evans/create-pull-request@v8`, …), **Dependabot-managed** (weekly version-updates + a cooldown) per [`SECURITY.md`](../SECURITY.md). (The zizmor advisory workflow and its `ref-pin` configuration were retired 2026-09-05 under agent-toolkit#3460. Two policies coexist and responders must not confuse them: this repository's committed workflows keep governed `@vN` tags under Dependabot cooldown as above, and the organization merge gate — which judges only a workflow file that a PR *changes* — accepts `@vN` for GitHub-owned actions but requires a 40-hex pin for any third-party action in the changed file. So during response do **not** SHA-pin an untouched workflow; the first PR that edits one of the third-party-using workflows will have to pin that action to a commit to pass the gate.) So `@vN` is the **policy, not a finding** — do **NOT** SHA-pin during response (it would violate the governed convention). Tag-retargeting risk is mitigated by Dependabot + the cooldown; during response, verify the `@vN` tag resolves to the **expected** release and check for an unexpected re-point, rather than converting to a SHA.
...
5. **CodeQL finding →** native triage; escalate by content (secret → §2; CI/supply-chain → §7). (The zizmor advisory workflow was retired 2026-09-05 under agent-toolkit#3460. It was path-scoped to workflow and config changes, so its 0 runs in 30 days were its designed idle state, not the reason; the reason is coverage without a consumer: it had 0 standing findings, nobody triaged its artifact, and the organization merge gate now refuses the merge-time class — `pull_request_target`, undeclared permissions, unpinned third-party actions, author-controlled text or `${{ secrets.* }}` in `run:` — on every changed workflow. The audits zizmor ran beyond that class — `artipacked`, `dependabot-cooldown`, `dangerous-triggers`, cache poisoning — now have **no automated owner** in this repository; run `zizmor --offline .` locally before editing a workflow, and treat a finding in those classes as a §7 supply-chain item.)
