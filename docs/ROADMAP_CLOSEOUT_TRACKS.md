...
| **B6 — Release/DOI/MCP/embeddings readiness checklist** | Consolidate E21/E22/E23/E34/E38 gated items into one readiness matrix | S | Automation |
| **B7 — Final audit harness/checklist** | Create the checklist and scripts for the final large audit (Section 6) | M | Planning |
| **B8 — Security/secrets review** | Verify `.gitleaks.toml` coverage, confirm no secrets in history, document rotation schedule | S | Automation |
...
| MCP/Ask/OG deployment gated | Code-reviewed | `.github/workflows/mcp-server.yml` deploy step lines 86–100 |
| IndexNow dry-run | CI-reviewed | `.github/workflows/build-and-deploy.yml` indexnow job lines 79–94 |
