...
\*\*Requirements:\*\* Claude Code v2.1.51 or later. Available on Pro, Max, Team, and Enterprise plans. Research preview.
...
\`\`\`
\# Enable inside an existing local Claude Code session:
/remote-control

\# Or spawn a new local session from the mobile app:
claude remote-control

\# Optionally name the local session for easy identification:
claude --name "Refactoring auth module"
\`\`\`
...
4\. You get push notifications when it finishes or needs input
5\. Results appear in the desktop sidebar with a "Dispatch" badge
...
There is no hard limit on the number of sessions, but each session consumes tokens from the same plan's usage pool. Practically, 3-5 parallel sessions is sustainable on the Max plan. More than that and you hit rate limits frequently.
...

{
 ...
 "text": "There is no hard limit on the number of sessions, but each session consumes tokens from the same plan's usage pool. Practically, 3-5 parallel sessions is sustainable on the Max plan. More than that and you hit rate limits frequently."
